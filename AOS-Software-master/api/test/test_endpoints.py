...
def test_led_off_mode_run(api_v1_host):
    endpoint = os.path.join(api_v1_host, 'led_off_mode', 'run')
    response = requests.get(endpoint)
    assert response.status_code == 200
    json = response.json()
    assert 'msg' in json
    assert 'led_on' in json

def test_led_on_mode_run(api_v1_host):
    endpoint = os.path.join(api_v1_host, 'led_on_mode', 'run')
    response = requests.get(endpoint)
    assert response.status_code == 200
    json = response.json()
    assert 'msg' in json
...
def test_test_mode_test(api_v1_host):
    endpoint = os.path.join(api_v1_host, 'test_mode', 'run')
    response = requests.get(endpoint)
    assert response.status_code == 200
    json = response.json()
    assert 'msg' in json
    assert json['msg'] == "The Test mode was succesfully started"
